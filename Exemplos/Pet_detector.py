...
def pet_detector(frame):
    ...
    if ((int(classes[0][0]) == 1) or (int(classes[0][0] == 2) or (int(classes[0][0]) == 3))) and (pause == 0):
    ...
    if pause == 1:
        ...
        if detected_outside == True:
            cv2.putText(frame, 'Pet wants inside!', (int(IM_WIDTH * .1), int(IM_HEIGHT * .5)), font, 3, (0, 0, 0), 7,
                        cv2.LINE_AA)
            cv2.putText(frame, 'Pet wants inside!', (int(IM_WIDTH * .1), int(IM_HEIGHT * .5)), font, 3, (95, 176, 23),
                        5, cv2.LINE_AA)
